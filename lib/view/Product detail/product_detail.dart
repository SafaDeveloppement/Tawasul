...
//             print("       Color Code: '${combo['code_coleur']}'");
//             print("       Stock: ${combo['qty'] ?? combo['stock']}");
...
//           "(Qty: ${combo.qty}, Available: ${combo.isAvailable}, "
...
//       final attributeId = _selectedColor?.id_product_attribute;
...
//           ),
//           SizedBox(width: 8.w),
//           if (_isColorAvailable && _selectedColor!.qty > 0)
//             Text(
//               '(${_selectedColor!.qty} available)',
...
//     );
//   }
//   Widget _buildColorDots(Product ColorModel) {
//     if (ColorModel.combinaisons.isEmpty) {
//       return const SizedBox.shrink();
//     }

//     return Row(
//       children:
//           ColorModel.combinaisons.map((combinaison) {
//             ColorModel color;
//             try {
//               color = ColorModel(
//                 int.parse(combinaison.codeColor.substring(1), radix: 16) +
//                     0xFF000000,
//               );
//             } catch (e) {
//               color = Colors.grey;
...
//     if (isLoading) {
//       return Scaffold(
//         backgroundColor: const ColorModel(0xFF008AD2),
//         body: Center(
//           child: Column(
//             mainAxisAlignment: MainAxisAlignment.center,
//             children: [
...
//                     foregroundColor: const ColorModel(0xFF008AD2),
...
//                                           height: 36.h,
//                                           decoration: BoxDecoration(
//                                             color: const ColorModel(0xFF008AD2),
//                                             shape: BoxShape.circle,
//                                           ),
//                                           child: Icon(
//                                             Icons.remove,
...
//                                                 colors: [
//                                                   ColorModel(0xFF008AD2),
//                                                   ColorModel(0xFF006DA9),
//                                                 ],
...
class _ProductDetailState extends State<ProductDetail> {
      ...
      if (result != null && result['success'] == true) {
        ...
        if (productData['combinations'] != null) {
          ...
          for (var i = 0; i < combos.length; i++) {
            ...
            print("  [$i] ID: ${combo['id_product_attribute']}");
            print("      Color Code: '${combo['code_coleur']}'");
            print("      Name: '${combo['name']}'");
            print("      Stock: ${combo['qty'] ?? combo['stock']}");
            print("      Default: ${combo['default_on']}");
          }
        }
        ...
        if (product!.colorCombinations.isNotEmpty) {
          ...
          for (var i = 0; i < product!.colorCombinations.length; i++) {
            final combo = product!.colorCombinations[i];
            print("  [$i] ${combo.colorName}");
            print("      Color Code: ${combo.colorCode}");
            print("      Available: ${combo.isAvailable}");
            print("      Quantity: ${combo.quantity}");
            print("      Is Default: ${combo.isDefault}");
          }
        } else {
          print("NO COLOR COMBINATIONS FOUND AFTER PARSING");
        }
  ...
  void _initializeColorSelection() {
    if (product != null && product!.hasColorCombinations) {
      ...
      for (var combo in colorCombos) {
        print(
          "  - ${combo.colorName}: ${combo.colorCode} "
          "(Qty: ${combo.quantity}, Available: ${combo.isAvailable}, "
          "Default: ${combo.isDefault})",
        );
      }
  ...
  void _addToCart() async {
    ...
    try {
      ...
      final cartProvider = Provider.of<CartProvider>(context, listen: false);
      final attributeId = _selectedColor?.idAttribute;
  ...
  Widget _buildColorDots() {
    ...
    final colorCombinations = product!.colorCombinations;
          ...
          child: Row(
            children:
                colorCombinations.map((combination) {
                  final isSelected =
                      _selectedColor?.idAttribute == combination.idAttribute;
                  final isAvailable = combination.isAvailable;

                  return Padding(
                    padding: EdgeInsets.only(right: 12.w),
                    child: _ColorDot(
                      color: combination.colorValue,
                      ...
                      onTap:
                          isAvailable
                              ? () {
                                setState(() {
                                  _selectedColor = combination;
                                  _isColorAvailable = combination.isAvailable;
                                  _updateImagesForSelectedColor();
                                });
                              }
                              : null,
                      tooltip:
                          '${combination.colorName}${!isAvailable ? ' (Out of Stock)' : ''}',
                    ),
                  );
                }).toList(),
          ),
